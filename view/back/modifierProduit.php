...
if (!$produit) { header('Location: afficherProduit.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom===''||strlen($nom)<2||$prix<=0||$prix>99999||$quantiteStock<0||$quantiteStock>99999||$dateExpiration==='') {
        ...
        header('Location: afficherProduit.php'); exit;
    }
}
...
<div class="page-body">
  <div class="form-card">
    <h3><i class="bi bi-pencil me-2" style="color:#e74c3c;"></i>Edit Product</h3>
    ...
    <form method="POST" enctype="multipart/form-data" class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <input type="text" name="nom" class="form-control" minlength="2" maxlength="100" value="<?= htmlspecialchars($produit['nom']) ?>" required>
      </div>
      <div class="col-md-6">
        <label class="form-label">Price (DT) *</label>
        <input type="number" step="0.01" min="0.01" max="99999" name="prix" class="form-control" value="<?= htmlspecialchars($produit['prix']) ?>" required>
      </div>
      <div class="col-md-6">
        <label class="form-label">Stock Quantity *</label>
        <input type="number" min="0" max="99999" name="quantiteStock" class="form-control" value="<?= (int)$produit['quantiteStock'] ?>" required>
      </div>
      <div class="col-md-6">
        <label class="form-label">Expiration Date *</label>
        <input type="date" name="dateExpiration" class="form-control" value="<?= htmlspecialchars($produit['dateExpiration']) ?>" required>
      </div>
      <div class="col-md-6">
        ...
        <select name="id_categorie" class="form-select">
          ...
          <?php foreach ($categories as $cat): ?>
            <option value="<?= (int)$cat['id_categorie'] ?>" <?= ((int)($produit['id_categorie']??0)===(int)$cat['id_categorie'])?'selected':'' ?>>
      ...
      <div class="col-md-6">
        ...
        <div class="mt-2">
          <?php
          $imgA = $produit['image'] ?? '';
          if ($imgA) { $imgS = str_starts_with($imgA,'http') ? $imgA : UPLOAD_URL.$imgA; }
          else { $imgS = ''; }
          ?>
          <img id="preview" src="<?= $imgS ? htmlspecialchars($imgS) : '#' ?>" alt="Preview"
               style="<?= $imgS ? '' : 'display:none;' ?>height:110px;object-fit:cover;border-radius:8px;border:1px solid #ddd;">
        </div>
      </div>
      <div class="col-12">
