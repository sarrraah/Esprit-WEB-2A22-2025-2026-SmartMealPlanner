...
$pageTitle = 'Pending Requests - SmartMeal Admin';
...
<style>
    ...
    .role-badge.coach       { background: #ecfdf3; color: #15803d; }
    .role-badge.nutritionist { background: #f5f3ff; color: #7c3aed; }
    ...
    @media (max-width: 768px) {
        .requests-head  { flex-direction: column; align-items: flex-start; }
        .request-top    { flex-direction: column; }
        .overview-box   { max-width: 100%; }
    }
</style>

<div class="admin-main">
    <div class="admin-topbar">
        <h5><i class="bi bi-person-check me-2" style="color:var(--accent)"></i>Pending Requests</h5>
    </div>
            ...
            <?php if (!empty($pendingUsers)): ?>
                <div class="requests-grid" id="requestsGrid">
                    <?php foreach ($pendingUsers as $index => $user): ?>
                        ...
                        $roleClass = in_array($role, ['coach', 'nutritionist']) ? $role : 'default';
                        $fullName  = trim(($user['prenom'] ?? '') . ' ' . ($user['nom'] ?? ''));
                        $email     = (string)($user['email'] ?? '');
                        $experience = (string)($user['experience'] ?? '—');
                        $speciality = (string)($user['speciality'] ?? '—');
                        ?>
                        <a href="review_user_request.php?id=<?= urlencode((string)$user['id']) ?>"
                           class="request-item"
                           ...
                           style="animation-delay: <?= 0.04 * $index ?>s;">
                            ...
                            <div class="detail-row">
                                ...
                                <div class="detail-value"><?= htmlspecialchars($experience) ?></div>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">Speciality</span>
                                <div class="detail-value"><?= htmlspecialchars($speciality) ?></div>
                            </div>
    ...
    function filterRequests() {
        ...
        cards.forEach(card => {
            const matchesSearch = (card.dataset.name || '').includes(searchValue) ||
                                  (card.dataset.email || '').includes(searchValue);
        ...
        if (noResultsState)  noResultsState.classList.toggle('hidden-card', shown !== 0);
    }
